...
interface TransitionHook {
  ...
  showOverlay: boolean;
  handleTransitionClick: (e: React.MouseEvent<HTMLAnchorElement>, path: string) => void;
}
...
const usePageTransition = (): TransitionHook => {
  ...
  const handleTransitionClick = useCallback((e: React.MouseEvent<HTMLAnchorElement>, path: string) => {
